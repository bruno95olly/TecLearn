...
import { SvgBronze, SvgCrown, SvgOuro, SvgPlatina, imageDefault } from "../components/Svg";
import { Alert, Avatar, Button, Checkbox, FormControl, FormControlLabel, FormGroup, FormLabel, InputBase, InputLabel, MenuItem, Modal, Radio, RadioGroup, Select, Stack, TextField } from "@mui/material";
import { Bookmark,  CommentOutlined, Edit, FavoriteBorder, Favorite, FilterAlt, Image, InsertDriveFile, Leaderboard, MoreHoriz, VideoLibrary, PhotoCamera, Send, Diamond } from "@mui/icons-material";
...
function Feed(){
    ...
    const postPub = () => {
        ...
        formData.append('tblUsuariosComunIdUsuarioComum', userID.tblUsuarioIdUsuario);
    ...
    const listPublicacaoCursoId = (id) => {
        PublicacaoService.getByCourseId(id).then(result => {
			...
			if (result instanceof Error) {
                ...
                result.forEach((row, i) => {publicacoesCursos.push(row.tblPublicaco);})
    ...
    return(
        <div className={styles.ContainerFeed}>
            <div>
                <div className="CardPost" style={{minHeight: "auto", width: "100%"}}>
                    <div className="ContentProfile">
                        <div className="CardHeader" style={{height: "80px"}}></div>
                        <div className="MyAvatar" style={{height: "4rem", width: "4rem"}}>
                            {userID.foto == "" ? userID && (imageDefault) : userID && (<img src={"http://localhost:8080/uploads/"+userID.foto}/>)}
                        </div>
                        <div className="ContainerInfo">
                            <div className="InfoTexts">
                                {userID && (<h1>{userID.apelido}</h1>)}
                    ...
                    <div className={styles.ContentFilters}>
                        ...
                        <div>
                            <FormControl sx={{
                                ...
                                <FormLabel id="demo-radio-buttons-group-label" sx={{fontSize: "1.2rem", color: colors.text, fontWeight: "500"}}>Cursos</FormLabel>
                                <RadioGroup
                                ...
                                >
                                {cursos.map((row, i) => (
                                    <FormControlLabel value={row.idCurso} control={<Radio onChange={()=>listPublicacaoCursoId(row.idCurso)}/>} label={row.cursoNome} title={row.cursoNome} />
                                ))}
                                </RadioGroup>
                            </FormControl> 
                        </div>
            ...
            <div>
                <div className="CardPost" style={{minHeight: "auto", width: "100%"}}>
                    <div className={styles.CreatePost}>
                            <div>
                                <div className="MyAvatar" style={{height: "3rem", width: "3rem"}}>
                                    {userID.foto == "" ? userID && (imageDefault) : userID && (<img src={"http://localhost:8080/uploads/"+userID.foto}/>)}
                                </div>
                                {userID && (<h1>{userID.apelido}</h1>)}
                            </div>
                            ...
                            >
                            <div className="CardPost" style={{minHeight: "10vh", width: "55vw"}}>
                                <div className={styles.CreatePost} style={{padding: "4% 4%"}}>
                                <h1 className={styles.TitleCreatePubli}>Criar Pergunta</h1>
                                    <input className={styles.QuestionCreatePubli} value={tituloPost} onChange={e=> setTituloPost(e.target.value)} placeholder="Titulo da pergunta" type="text" />
                                    <InputBase
                                    className={styles.DescCreatePubli} 
                                    multiline
                                    onChange={(e)=>{setDescricao(e.target.value)}}
                                    maxRows={4}
                                    placeholder="Digite sua pergunta aqui"
                                    />
                                    <InputImageFeed img={img} setImg={setImg}/>
                                    <div className={styles.ContainerAnexos}>
                                        <div className={styles.BoxAnexos}>
                                        <label htmlFor="file-image">
                                            <span><Image/>Imagem</span>
                                        </label>
                                        </div>
                                    </div>
                                    <div className={styles.BoxButtonPublicar}>
                                        <FormControl variant="standard" sx={{ m: 1, minWidth: 120}}>
                                            <InputLabel id="demo-simple-select-standard-label">Curso</InputLabel>
                                            <Select
                                            ...
                                            >
                                            {cursos.map((row, i)=>(
                                                <MenuItem value={row.idCurso}>
                                                    <em>{row.cursoNome}</em>
                                                </MenuItem>
                                            ))}
                                            </Select>
                                        </FormControl>
                                        <Button onClick={postPub} sx={{marginTop: 0, textAlign: "end", fontSize: 18, width: "40%"}} variant="contained">Publicar</Button>
                                    </div>
                                </div>
                            </div>
                            </Modal>
                            ...
                            <div>
                                <div className={styles.BoxAnexos}>
                                    <span onClick={handleOpen}><Image/>Imagem</span>
                                </div>
                            </div>
                    </div>
                </div>
                {publicacoes.map((row, i) => {
                    return(
                        <CardPublicacao row={row} isTeatcher={isTeatcher} listPublicacaoCursoId={listPublicacaoCursoId} comentarioPub={comentarioPub} userID={userID.tblUsuarioIdUsuario} idPublicacao={row.idPublicacao}/>    
                    )
                    }
                )}
            ...
            <div>
                <div className="CardPost" style={{minHeight: "40vh", width: "100%"}}>
                            ...
                            return(
                                ...
                                {row && (<h3>{i+1+"º"}</h3>)}
                                    <div className={styles.AvatarRanking}>
                                        <div className="MyAvatar" style={{height: "42px", width: "42px", margin: 0}}>
                                            {row.foto == "" ? row && (imageDefault) : row && (<img src={"http://localhost:8080/uploads/"+row.foto}/>)}
                                        </div>
                                        {row && (<p>{row.tblUsuario.nome}</p>)}
                                        {row && (<p style={{color: '#c000ff', display: 'flex', alignItems: 'center'}}><Diamond sx={{verticalAlign: "bottom", marginRight: 1}}/>{row.pontuacao}</p>)}
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div className="CardPost" style={{minHeight: "50vh", width: "100%"}}>
